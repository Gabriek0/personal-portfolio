...
export const mdxComponents: MDXComponents = {
  ...
  ul: ({ children }) => (
    <ul className='list-disc pl-6 mb-5 space-y-2 text-foreground'>{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className='list-decimal pl-6 mb-5 space-y-2 text-foreground'>{children}</ol>
  ),
  li: ({ children }) => <li className='text-base leading-[1.75]'>{children}</li>,
  strong: ({ children }) => (
